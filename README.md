...
A single phase motor only has 2 methods of control: amplitude and phase. Here, we keep amplitude at max, then the phase shift is tuned until max speed and efficiency is achieved. 
...
3. So I skipped PWM peripheral to remove the delay before the it fires and simply fullsend the motor ie it is stuck at max speed
...
5. I found that at a certain phase shift (coincidentally where the motor is at max speed) the mosfets run alot cooler which leads me to assume some zero voltage/current switching is occuring
6. assuming 20khz input frequency (at 120,000 rpm) theres 800 clock cycles between each INT0 & TIM1 interrupt which really doesnt leave much room for interrupt handling 
...
WARNING: the whole of PORTC is switched together to save cycles since those extra pins arent being used
...
step 3: to speed it up, phase shift the drive waveform a bit forward, this is accomplished by settomg timer1 to fire a little earlier than the previous edge


Hacks: reset function and lazy virtual ground/supplies
